...
// Buat Variabel untuk tahun dan namaorganisasi untuk menampilkan data berdasarkan kriteria
    ...
    $categories = array('Kelurahan');

// data series tampilan di grafik
 
    $data_series = array('Kandai', 'Kemaraya');
    $data_series2 = array('l1', 'l2');
    ...
    foreach ($data_series2 as $key=>$val) {
		...
		$y=mysql_fetch_row($x);
		$sql = "SELECT SUM(total_penjualan) AS result FROM fact_tabel_penjualan WHERE kode_lokasi = '$val' AND kode_dimensi = '$y[0]'";
...
$('#contoh').highcharts({
    ...
    title: {
        text: 'TOTAL PENJUALAN BARANG BERDASARKAN KELURAHAN PER TAHUN'
    },
    subtitle: {
        text: '<?php echo $year; ?>'
    },
